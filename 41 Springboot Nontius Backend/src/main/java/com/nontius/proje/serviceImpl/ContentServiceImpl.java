...
public class ContentServiceImpl implements ContentService{
	...
	public List<String> getAllContentUrls() {
        return repository.findAll()
                                .stream()
                                .map(content -> "http://nontius.com/readContent/" + toSlug(content.getTitle()))
                                .collect(Collectors.toList());
    }

	private String toSlug(String title) {
	    String turkishChars = title
	        .replace("ğ", "g").replace("ı", "i").replace("ş", "s")
	        .replace("ç", "c").replace("ö", "o").replace("ü", "u")
	        .replace("Ğ", "G").replace("İ", "I").replace("Ş", "S")
	        .replace("Ç", "C").replace("Ö", "O").replace("Ü", "U");

	    return turkishChars.toLowerCase()
	                       .replaceAll("[^a-z0-9]+", "-")
	                       .replaceAll("^-|-$", "");
	}
